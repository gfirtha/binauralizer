classdef lwfs_renderer_nara < base_renderer
    ...
    methods
        function obj = lwfs_renderer_nara(virtual_source,SSD, fs)
        ...
        function obj = update_renderer(obj, ~)
            obj;
            Nt = length(obj.virtual_source.source_signal.time_series);
            obj.t = (-0:Nt-1)'/obj.fs;
            x0 = cell2mat(cellfun( @(x) x.position,    obj.secondary_source_distribution, 'UniformOutput', false)');
            fi0 = cart2pol(x0(:,1),x0(:,2));
            n0 = cell2mat(cellfun( @(x) x.orientation, obj.secondary_source_distribution, 'UniformOutput', false)');
            rref = sqrt(sum((x0-obj.xref).^2,2));
%            obj.c*sqrt(2*pi/rref)
            kPw = bsxfun( @minus, [0,0], obj.virtual_source.position );
            [phi_pw,r] = cart2pol(kPw(1),kPw(2));
            phi_pw  = phi_pw- pi;

            Ma = 5;
            ma = (-Ma:Ma)';
            Sm = exp(-1i*ma*phi_pw);
            [~,Am] = fourier_series_wfs_window(Ma, phi_pw);
            Dm = conv(Am,Sm);
            ct = obj.c*obj.t./rref';
            ix_invalid = abs(ct)>1;
            M = Ma*2;
            ...
            for m = -M : M
                m
                Tm = chebyshevT(abs(m),ct);
                Tm(ix_invalid) = 0;
                Dwfs = Dwfs + Dm(m+M+1)*obj.c*sqrt(8./(pi*rref.')).*Tm./sqrt(1-ct.^2).*exp(1i*m*fi0.');
            end
            Dwfs = real(Dwfs);
%             M = Ma*2;
%             N = 2*M;
...
%             P = 0;
%             Dwfs = 0;
%             Anm = xyplane_sht_coeff(N,M)   
%             for m = -M:M
%                  for n = abs(m):N
%                     P = P +  Pn(:,:,n+1).*(2*n+1).  *Anm{n+1}(get_circular_index(m,Anm{n+1}));
%                  end
%                  Dwfs = Dwfs + P.*(Dm(m+M+1).*exp(1i*m*fi0)).';
%             end
           

            for n = 1 : length(obj.WFS_filter_bank)
                obj.WFS_filter_bank{n}.update_coefficients(Dwfs(:,n));
            end
        end
        ...
        function [amp,delay] = get_delay_and_gain(obj)
            ...
            rho_G = sqrt(sum((x0-obj.xref).^2,2));
        ...
        function get_antialiasing_filters(obj,N)
            ...
            d = sqrt(sum((obj.xref-x0).^2,2));
            [~,ix_stat] = min(abs(sum((x0 + d.*kP - obj.xref).^2,2)));
