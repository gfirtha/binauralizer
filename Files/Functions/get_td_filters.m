function [td_filters] = get_td_filters( xs, x0, n0, fs, c )
%xs = [2.5 0.5];
xr = [0 0];
N  = 512;
w = [ 0 : N-1 ]'/N*2*pi*fs;
...
if max(abs(vn)) == 1
    ixs = ix(1);
    td_filters(:,ixs) = fftshift(ifft(ones(N,1),N,'symmetric'));
   ...
   arg = atan( w/c*R0*tan(fi0)/tan(fi_base) );
   H_sp1 = exp(+1i*arg);
   H_sp2 = exp(-1i*arg);
...
%      xlim([20,2e3])
%      
    td_filters(:,ixs(1)) = real(fftshift(ifft(H_sp1_lin,N,'symmetric')));
    td_filters(:,ixs(2)) = real(fftshift(ifft(H_sp2_lin,N,'symmetric')));
end
